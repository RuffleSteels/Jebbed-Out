...
public class ConfigScreen extends Screen {
    ...
    @Override
    protected void init() {
        customTimeSliderWidget = new Slider(this.width / 2 - 75, height/2 - 23, 150, 20, Text.translatable("jebbed.config.title.speed"), Main.CONFIG_MANAGER.getJebbedSpeed());this.addDrawableChild(customTimeSliderWidget);
        this.addDrawableChild(customTimeSliderWidget);
        this.addDrawableChild(new ButtonWidget.Builder(ScreenTexts.DONE, button -> close()).dimensions(width / 2 - 100, height/2 + 3, 200, 20).build());super.init();
        super.init();
    }
    @Override
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        Identifier id = Identifier.of("jebbed:textures/gui/warning.png");
        ...
        bufferBuilder.vertex(matrix4f, (float)x, (float)y+h, (float)z).texture(u1, v2).color(1.0f, 1.0f, 1.0f, alpha);
        bufferBuilder.vertex(matrix4f, (float)x+spriteWidth, (float)y+h, (float)z).texture(u2, v2).color(1.0f, 1.0f, 1.0f, alpha);
        bufferBuilder.vertex(matrix4f, (float)x+spriteWidth, (float)y, (float)z).texture(u2, v1).color(1.0f, 1.0f, 1.0f, alpha);
